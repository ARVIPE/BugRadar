...
UPTIME_API_URL = os.getenv("BUGRADAR_UPTIME_API_URL", "http://localhost:3000/api/uptime")
LATENCY_API_URL = os.getenv("BUGRADAR_LATENCY_API_URL", "http://localhost:3000/api/latency")

UPTIME_TARGET_URL = os.getenv("BUGRADAR_UPTIME_TARGET_URL")
...
UPTIME_EVERY = int(os.getenv("BUGRADAR_UPTIME_EVERY", "60"))
...
def check_uptime():
    if not UPTIME_TARGET_URL: return
    try:
...
def uptime_loop():
    if not UPTIME_TARGET_URL: return
    print("⏱️  Starting uptime monitor...")
    while True:
        try:
            check_uptime()
